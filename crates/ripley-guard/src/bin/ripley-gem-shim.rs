...
fn main() -> ExitCode {
    ...
    let real_gem = match find_real_gem() {
        Some(path) => path,
        None => {
            eprintln!("ripley: could not find real gem in PATH");
            return ExitCode::from(2);
        }
    };

    let status = Command::new(&real_gem).args(&args).status();

    match status {
        Ok(s) => ExitCode::from(s.code().unwrap_or(1) as u8),
        Err(e) => {
            eprintln!("ripley: could not execute gem: {e}");
            ExitCode::from(2)
        }
    }
}
...
fn check_advisories(package: &str) {
    ...
    let advisories = match db.get_advisories(ripley_core::types::Ecosystem::Gem, package) {
        Ok(a) => a,
        Err(_) => return,
    };
...
fn find_real_gem() -> Option<String> {
    let self_exe = std::env::current_exe().ok()?;
    let self_dir = self_exe.parent()?;

    let output = Command::new("which").arg("-a").arg("gem").output().ok()?;
