...
fn main() -> ExitCode {
    ...
    let real_pnpm = match find_real_pnpm() {
        Some(path) => path,
        None => {
            eprintln!("ripley: could not find real pnpm in PATH");
            return ExitCode::from(2);
        }
    };

    let status = Command::new(&real_pnpm).args(&args).status();

    match status {
        Ok(s) => ExitCode::from(s.code().unwrap_or(1) as u8),
        Err(e) => {
            eprintln!("ripley: could not execute pnpm: {e}");
            ExitCode::from(2)
        }
    }
}
...
fn is_install_command(args: &[String]) -> bool {
    args.first()
        .is_some_and(|cmd| matches!(cmd.as_str(), "install" | "i" | "add"))
}
...
fn find_real_pnpm() -> Option<String> {
    let self_exe = std::env::current_exe().ok()?;
    let self_dir = self_exe.parent()?;

    let output = Command::new("which").arg("-a").arg("pnpm").output().ok()?;
