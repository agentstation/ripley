...
fn main() -> ExitCode {
    ...
    let real_cargo = match find_real_cargo() {
        Some(path) => path,
        None => {
            eprintln!("ripley: could not find real cargo in PATH");
            return ExitCode::from(2);
        }
    };

    let status = Command::new(&real_cargo).args(&args).status();

    match status {
        Ok(s) => ExitCode::from(s.code().unwrap_or(1) as u8),
        Err(e) => {
            eprintln!("ripley: could not execute cargo: {e}");
            ExitCode::from(2)
        }
    }
}
...
fn is_install_command(args: &[String]) -> bool {
    args.first()
        .is_some_and(|cmd| matches!(cmd.as_str(), "build" | "install"))
}

fn extract_package_name(args: &[String]) -> Option<String> {
    // Only extract a package name for `cargo install <crate>`.
    // `cargo build` compiles from local source, so there is no remote package to check.
...
fn check_advisories(package: &str) {
    ...
    let advisories = match db.get_advisories(ripley_core::types::Ecosystem::Cargo, package) {
        Ok(a) => a,
        Err(_) => return,
    };
...
fn find_real_cargo() -> Option<String> {
    let self_exe = std::env::current_exe().ok()?;
    let self_dir = self_exe.parent()?;

    let output = Command::new("which").arg("-a").arg("cargo").output().ok()?;
