...
fn main() -> ExitCode {
    ...
    let real_pip = match find_real_pip() {
        Some(path) => path,
        None => {
            eprintln!("ripley: could not find real pip in PATH");
            return ExitCode::from(2);
        }
    };

    let status = Command::new(&real_pip).args(&args).status();

    match status {
        Ok(s) => ExitCode::from(s.code().unwrap_or(1) as u8),
        Err(e) => {
            eprintln!("ripley: could not execute pip: {e}");
            ExitCode::from(2)
        }
    }
}
...
fn is_install_command(args: &[String]) -> bool {
    args.first()
        .is_some_and(|cmd| matches!(cmd.as_str(), "install"))
}
...
fn check_advisories(package: &str) {
    ...
    let advisories = match db.get_advisories(ripley_core::types::Ecosystem::PyPI, package) {
        Ok(a) => a,
        Err(_) => return,
    };
...
fn find_real_pip() -> Option<String> {
    let self_exe = std::env::current_exe().ok()?;
    let self_dir = self_exe.parent()?;

    let output = Command::new("which").arg("-a").arg("pip").output().ok()?;
