...
|----------|--------|-----------|
| Language (core, CLI, desktop backend) | Rust | The core, CLI, and Tauri backend need to be fast, dependency-light, and cross-platform. The guard runs on every `npm install` — it must add < 200ms latency. |
| Language (desktop frontend) | TypeScript 6 | Industry-standard frontend language; React 19 ecosystem alignment; type-safe IPC via `tauri-specta` codegen end-to-end. TS 7 (Go-native port) still beta — defer. |
| Cross-platform GUI v1 | macOS + Linux + Windows from first release | The threat model (compromised packages) is platform-agnostic; the CLI is already cross-platform; the GUI cannot be the platform bottleneck. Retrofitting cross-platform after building macOS-only is the most expensive failure mode and is explicitly rejected. Documented in full in [STACK_DECISION.md](STACK_DECISION.md). |
| Desktop framework | Tauri 2.11 | Cross-platform first-class (macOS/Linux/Windows). First-party tray (`TrayIconBuilder` per-OS), DOM accessibility tree for Peekaboo design loop, React + Tailwind component ecosystem. Replaces the Phase 1-5 iced stack (retired in Phase 6 M28). Slint was the credible alternative but AccessKit gap broke the Peekaboo loop and DSL hireability is worse than React. See [STACK_DECISION.md](STACK_DECISION.md). |
| Tray | Tauri `TrayIconBuilder` | One API across `NSStatusItem` (macOS), `Shell_NotifyIcon` (Windows), `StatusNotifierItem`/libappindicator (Linux). Replaces `tray-icon` + `muda` (Phase 1-5) which had no first-party path with iced. |
| Frontend runtime | React 19.2 | Industry-standard SPA library; React 19 stable GA; designer/contractor hireability is dramatically better than DSL alternatives (Slint, GPUI). Server Components are irrelevant for a Tauri SPA. |
| Frontend build tool | Vite 8 | Tauri-default; Rolldown integrated, Oxc replaces Babel. HMR + bundling in one tool. |
| Frontend styling | Tailwind v4 with `@theme` block | Utility-first matches DESIGN.md's token-heavy spec. v4 moved config into CSS — `@theme` block is the right home for our palette. No `tailwind.config.ts`. |
| Frontend components | shadcn/ui (Base UI primitive `@base-ui/react` 1.x) | Copy-into-repo ownership model; Base UI maintained by MUI team (Radix slowed after WorkOS acquisition); cleaner render-prop API than Radix's `asChild`; native search Combobox eliminates need for cmdk. Style: `base-vega`. Decision detail in [STACK_DECISION.md](STACK_DECISION.md). |
| Frontend state (IPC) | TanStack Query v5 | Wraps Tauri invokes as "server state": caching, retries, mutations, invalidation, devtools. Treats every Tauri command as a cacheable async resource. Invalidate on Tauri event receipt. |
| Frontend state (UI) | Zustand 5 | Pure client state (selected view, expanded sections, modal open/closed). Slice pattern at >2 slices. No providers, TS-native. Do **not** mirror server state into Zustand — that's Query's job. |
| Type-safe IPC | `tauri-specta` v2 | Codegen TS types from `#[tauri::command]` + `specta::Type`-derived structs. No hand-written `invoke<T>("cmd")` calls. Generated bindings committed to repo (same principle as `Cargo.lock`). Adopt day one or rewrite later. |
| Frontend lint | ESLint 9 flat config | Biome 2.4 still missing `react-hooks/exhaustive-deps` and security plugin breadth (`eslint-plugin-security`, `no-secrets`) — disqualifying for a supply-chain tool. Revisit when Biome closes the gap. |
| Frontend unit tests | Vitest 2.x | Vite-aligned default; native ESM; React 19 / Suspense v2 ready. Snapshot testing for component output (analogous to `insta` for Rust). |
| Frontend e2e tests | WebdriverIO 9 + `tauri-driver` | Tauri 2's officially documented path. Playwright doesn't support WKWebView and is not Tauri-supported. macOS gap (WKWebView doesn't expose WebDriver reliably) is accepted: Linux + Windows CI runs e2e, macOS covered by Vitest + Peekaboo screenshots. |
| Package manager (JS) | pnpm | Strict `node_modules` layout catches phantom deps — critical for a supply-chain tool. npm + yarn forbidden. |
| Git hooks | Lefthook 1.x | Single Go binary, parallel execution, language-agnostic. No Node post-install side effect — important when Rust-only contributors don't have Node installed. Native `{staged_files}` glob makes `lint-staged` redundant. |
| Versioning (Rust) | `release-plz` + Conventional Commits | Workspace-aware; drives `Cargo.toml` bumps, CHANGELOG, git tags, GitHub releases from Conventional Commits. Single source of truth for desktop app + CLI version. |
| Task runner | `just` | Rust-ecosystem-canonical; small Rust binary; clean arg syntax. Top-level `just dev`/`build`/`test`/`check` proxy to the right toolchain. |
| Tool version pinning | `mise` | Pins node + pnpm + just + lefthook + tauri-cli. Supply-chain hygiene — pinned tools, reproducible environments. |
| JS supply chain audit | `pnpm audit` + `osv-scanner` in CI | A supply-chain tool must audit its JS deps the same way it audits its Rust deps. |
...
| Binary structure | Separate CLI + desktop app | `ripley` CLI binary (built from `ripley-guard` crate via `[[bin]] name = "ripley"`) works standalone: scan, guard, watch, status, config, fix, exposure, audit, harden. Desktop app (from `apps/desktop/src-tauri`, bundled by Tauri) runs as a tray-resident background app on all 3 OSes. `ripley watch` starts a headless daemon with the same polling/matching logic but no UI. CLI communicates with desktop app over UDS when running, falls back to direct operation when not. Follows Docker Desktop, Tailscale, 1Password pattern. |
...
| App bundling | Tauri bundler | One bundler, three OSes: `.dmg` + `.app` (macOS, with `LSUIElement = true` for tray-only / no Dock), `.msi` + `.exe` (Windows, signed), AppImage + `.deb` + `.rpm` (Linux). Replaces `cargo-bundle` (Phase 1-5). Code signing: Apple Developer ID hardened runtime + notarization; Windows EV cert; Tauri updater ships Ed25519-signed manifests (Sparkle-equivalent integrity model). |
