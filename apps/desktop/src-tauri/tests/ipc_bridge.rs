...
#[tokio::test]
async fn bridge_rejects_malformed_json() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let socket_path = tmp.path().join("ripley.sock");
    let pending = ipc_bridge::new_pending();
    let latencies = ipc_bridge::new_latency_map();
...
#[tokio::test]
async fn bridge_cleans_up_pending_when_client_disconnects_before_decision() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let socket_path = tmp.path().join("ripley.sock");
    let pending = ipc_bridge::new_pending();
    let latencies = ipc_bridge::new_latency_map();
    ...
    let _id = wait_for_event(&captured).await;
    drop(client);
