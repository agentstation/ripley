...
### Component 1: `ripley-desktop` --- the tray app

Built with **Tauri 2** (Rust backend + WebView frontend). The frontend is React 19
+ shadcn/ui (Base UI primitive) + Tailwind v4 + Zustand 5 + TanStack Query v5,
bundled by Vite 8. The system tray is Tauri's first-party `TrayIconBuilder`,
backed by `NSStatusItem` on macOS, `Shell_NotifyIcon` on Windows, and
`StatusNotifierItem` / libappindicator on Linux — a single API across all three
platforms.

Cross-platform GUI is a v1 requirement (macOS + Linux + Windows from first
release). Type-safe IPC between the Rust backend and TS frontend is generated by
`tauri-specta` v2 — `#[tauri::command]` handlers and `specta::Type`-derived DTOs
emit `apps/desktop/src/lib/bindings.ts` at build time.

Packaged via Tauri's built-in bundler: `.dmg` + `.app` (macOS, notarized via
Developer ID hardened runtime), `.msi` + `.exe` (Windows, signed), AppImage +
`.deb` + `.rpm` (Linux). The desktop app communicates with the `ripley` CLI via
a Unix domain socket at `{data_dir}/ripley.sock` using JSON-over-socket IPC ---
the same pattern used by Docker, Tailscale, and 1Password. The UDS protocol is
unchanged from Phase 1-5; only the GUI consumer is new.

Clicking the tray icon opens a pre-warmed (hidden, invisible-since-startup)
dashboard window showing alerts, guard activity, and settings — first show
budgets <500ms, subsequent shows <50ms. Native OS notifications fire for new
alerts and hand off to the dashboard for detail and action. See [UI.md](UI.md)
for view wireframes, [DESIGN.md](DESIGN.md) for the design system tokens, and
[STACK_DECISION.md](STACK_DECISION.md) for the full UI stack rationale.

The previous iced-based `crates/ripley-app` (Phases 1-5) is retired in Phase 6
M28 once the Tauri app reaches feature parity.
...
| Area | Choice |
|------|--------|
| Language (core, CLI, desktop backend) | Rust 1.84+ (resolver = "3") |
| Language (desktop frontend) | TypeScript 6 |
| Desktop framework | Tauri 2.11 (cross-platform: macOS + Linux + Windows) |
| Tray | Tauri `TrayIconBuilder` (NSStatusItem / Shell_NotifyIcon / StatusNotifierItem) |
| Frontend runtime | React 19.2 (SPA, no SSR/RSC) |
| Frontend build tool | Vite 8 (Rolldown + Oxc) |
| Frontend styling | Tailwind v4 with `@theme` block (no `tailwind.config.ts`) |
| Frontend components | shadcn/ui (Base UI primitive `@base-ui/react` 1.x, style `base-vega`) |
| Frontend state (IPC) | TanStack Query v5 wrapping `tauri-specta` commands |
| Frontend state (UI) | Zustand 5 (slice pattern) |
| Frontend tables | TanStack Table v8 + TanStack Virtual v3 |
| Frontend fuzzy search | `match-sorter` |
| Frontend icons | `lucide-react` |
| Type-safe IPC | `tauri-specta` v2 (codegen from `#[tauri::command]` + `specta::Type`) |
| Frontend lint | ESLint 9 flat config + `typescript-eslint` v8 + React + jsx-a11y |
| Frontend format | Prettier 3 |
| Frontend unit tests | Vitest 2.x + `@testing-library/react` 16+ |
| Frontend e2e tests | WebdriverIO 9 + `tauri-driver` (Linux + Windows; macOS via Vitest + Peekaboo) |
| Package manager (JS) | pnpm (npm/yarn forbidden) |
| IPC (CLI ↔ desktop) | JSON over Unix domain socket (`{data_dir}/ripley.sock`) — unchanged from Phase 1-5 |
| App bundling | Tauri bundler (`.dmg` + `.app`, `.msi` + `.exe`, AppImage + `.deb` + `.rpm`) |
| Code signing / notarization | Apple Developer ID + hardened runtime; Windows EV cert; Tauri updater with Ed25519 signed manifests |
| Notifications | Tauri `notification` plugin (cross-platform native) |
...
| Async runtime | `tokio` (multi-threaded) + `mpsc` + `CancellationToken` |
| Platform directories | `directories` crate |
| Rust testing | `insta` snapshots, `cargo-fuzz` for parsers |
| Git hooks | Lefthook 1.x |
| Versioning | `release-plz` + Conventional Commits |
| Task runner | `just` |
| Tool version pinning | `mise` (node + pnpm + just + lefthook + tauri-cli) |
| Own supply chain | `cargo-deny` + `pnpm audit` + `rust-toolchain.toml` + `.mise.toml` |
...
Monorepo with Cargo workspace at root (load-bearing) and Tauri 2 desktop app
embedded under `apps/desktop/` as a workspace member. The Rust core
(`crates/ripley-core`, `crates/ripley-ipc`, `crates/ripley-guard`) is unchanged
from Phase 1-5. The iced `crates/ripley-app` is retired in Phase 6 M28.

See [STACK_DECISION.md](STACK_DECISION.md) for the locked repo layout reference;
this section is the canonical project structure spec.
...
├── release-plz.toml                # Rust release automation (Conventional Commits)
├── .mise.toml                      # pins node, pnpm, just, lefthook, tauri-cli
├── package.json                    # private root; scripts proxy to `just`
...
│   │   └── src/
│   │       ├── main.rs             # `ripley` binary: scan, guard, watch, status, config
...
│   │       └── bin/
│   │           └── ripley-script-shell.rs   # script-shell binary; speaks UDS to desktop
...
│       └── src/                    #   shim generation
│           └── main.rs
│   # Note: crates/ripley-app/ (iced) is being retired in Phase 6 M28
│
├── apps/                           # END-USER PRODUCTS
│   └── desktop/                    # Tauri 2 desktop app
│       ├── package.json            # React 19, shadcn, Tailwind v4, Zustand, Query
...
│       │   ├── lib/
│       │   │   ├── bindings.ts     # tauri-specta GENERATED — committed
...
│       │   ├── Cargo.toml          # name = "ripley-desktop"
...
│       │
│       └── tests/                  # WebdriverIO e2e (Linux + Windows CI)
│
├── packages/                       # FUTURE shared JS packages (scaffolded, empty)
│   └── .gitkeep
│
...
The root `Cargo.toml` is a workspace with `resolver = "3"` (Rust 1.84+). Shared
dependencies are declared once in `[workspace.dependencies]` and inherited by
each crate via `{ workspace = true }`. Shared package metadata (`version`,
`license`, `edition`, `rust-version`) lives in `[workspace.package]` and members
declare `version.workspace = true`, etc. This keeps versions synchronized across
`ripley-core`, `ripley-guard`, `ripley-ipc`, and `ripley-desktop` (the Tauri
backend crate).
...
The `apps/desktop/src-tauri` crate produces `Ripley.app` on macOS, `Ripley.exe`
on Windows, and `ripley-desktop` on Linux via Tauri's bundler. `ripley` (no
subcommand) launches the desktop app if installed, otherwise prints help. This
follows the pattern of `code` (launches VS Code) and `docker` (routes to Docker
