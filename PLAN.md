...
- [ ] `pnpm install && pnpm -C apps/desktop tauri dev` launches tray-only app on macOS  *(visual smoke deferred to M28 release-cut per DESIGN_NOTES.md)*
- [ ] No Dock icon; tray icon visible; Cmd+Shift+R toggles hidden pre-warm window  *(visual smoke deferred to M28 release-cut per DESIGN_NOTES.md)*
- [x] `apps/desktop/src/lib/bindings.ts` generated by `cargo build -p ripley-desktop`
- [x] shadcn Button renders with DESIGN.md tokens (`base-vega` style + Base UI primitive)
- [x] `just check` passes (cargo fmt + clippy + test, pnpm lint + tsc + vitest, pnpm audit, cargo deny check)
- [x] Lefthook pre-commit gates pass on a noop commit
- [x] `crates/ripley-app` still builds and runs unchanged
